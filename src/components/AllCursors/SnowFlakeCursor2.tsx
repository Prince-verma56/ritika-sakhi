...
interface SnowflakeCursor2Props {
  element?: HTMLElement;
}
...
const SnowflakeCursor2: React.FC<SnowflakeCursor2Props> = ({ element }) => {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const particles = useRef<Particle2[]>([]);
  ...
  useEffect(() => {
    ...
    const addParticle = (x: number, y: number) => {
      ...
      particles.current.push(new Particle2(x, y, randomImage));
    };
    ...
    const updateParticles = () => {
      ...
      particles.current.forEach((particle, index) => {
        ...
        if (particle.lifeSpan < 0) particles.current.splice(index, 1);
      });
    };
    const animationLoop = () => {
      updateParticles();
    ...
    if (prefersReducedMotion.current) {
      prefersReducedMotion.current.onchange = () => {
        if (prefersReducedMotion.current?.matches) destroy();
        else init();
      };
    }
    init();
