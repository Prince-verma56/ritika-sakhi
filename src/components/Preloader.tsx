...
import { motion } from 'motion/react'; // Kept your motion/react import path
...
export default function Preloader({ onComplete }: { onComplete?: () => void }) {
  ...
  useEffect(() => {
    if (stepIdx < STEPS.length) {
      ...
      // 1. Hold at 100% for a brief moment before beginning the lift sequence
      const triggerExitTimer = setTimeout(() => {
        setIsExiting(true);
        
        // 2. The curtain lift takes 1.25 seconds. Wait exactly 1250ms before unmounting.
      ...
      return () => clearTimeout(triggerExitTimer);
    }
  }, [stepIdx, onComplete]);
  ...
  return (
    <motion.div
      className="fixed inset-0 z-[10000] flex flex-col justify-center items-center overflow-hidden bg-[#FCFAF2] pointer-events-auto"
      style={{ fontFamily: 'Georgia, serif' }}
      initial={{ y: '0%' }}
      animate={isExiting ? { y: '-100%' } : { y: '0%' }}
      transition={{ 
        duration: 1.25, 
        ease: [0.76, 0, 0.24, 1] // Custom premium curtain slide deceleration curve
      }}
    >
      ...
      <motion.div 
        className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-[55%] flex flex-col items-center gap-6 z-[5]"
        animate={isExiting ? { opacity: 0.4, scale: 0.96 } : { opacity: 1, scale: 1 }}
      ...
      <div 
        className="absolute bottom-[110px] left-8 right-8 md:left-16 md:right-16 h-[1px] z-[5]" 
      ...
      <div className="absolute bottom-8 md:bottom-12 left-0 right-0 flex items-end justify-between px-8 md:px-16 z-[5]">
